...
Daneben gibt es einen irreversiblen Wärmeverlust, dessen Größe durch den Innenwiderstand bestimmt ist. Im Innenwiderstand $R_i$ werden die Widerstände von ELektroden, Elektrolyt und der irreversible Anteil der Zellenraktion zusammengefasst.
\begin{equation}\label{gl_innenwiderstand}
Q_{irr}=-I^2\cdot R_i
\end{equation}

Das thermische Verhalten wir durch die Summe der reversiblen und irreversiblen Wärmeentwicklung beschrieben. Der reversible Anteil hängt linear vom Strom ab, der irreversible quadratisch. Für niedrige Ströme kann der reversible Anteil den irreversiblen übertreffen, das heißt die Batterie kann sich beim langsamen laden oder entladen \emph{abkühlen}. Bei hohen Strömen überwiegt der irreversible Anteil, bei Lithium-Ionen Akkus entpricht der reversible Anteil bei einer 1$C$-Entladung maximal 50\% des irreversiblen Anteils~\cite{Viswanathan20103720}.
...
Die Nickel-Cadmium-Batterie weist eine weit höhere spezifische Energie als die Blei-Säure-Batterie auf, wird jedoch in elektrischen Bussen nicht verwendet. \marginpar{Warum? Wirklich?}
...
\section{Parameter}
Die Bewertung der Speichersysteme erfolgt anhand der Ausgabedaten einer Verbrauchssimulation. In diesem Abschnitt werden die Kenndaten der Energiespeicher erläutert. Sie sind Eingangsdaten der Verbrauchsimulation. Die Verbrauchssimulation und ihre Ausgabedaten werden in Kapitel \ref{chap4} erläutert. 

Elektrochemische Batterien werden mit einem Batteriemodell simuliert, Kondensator und Schwungrad werden als generische Energiespeicher mit konstanten Leistungsdaten und variabler Effizienz betrachtet.

\subsection{Parameter für Batterien}
\begin{description}
	...
	\item[Entladestrom] Der maximale Entladestrom.
	\item[Gewicht] Das Gewicht einer Batterieinheit, inklusive Gehäuse und Verbindern.
	\item[Volumen] Das Packvolumen einer Batterieinheit. Bei Zylindrischen Zellen Beispielsweise $\frac{2\sqrt{3}}{\pi} \cdot V_{Zelle}$.
\end{description}

\subsection{Parameter für Kondensatoren und Schwungräder}
\begin{description}
	\item[Energiedichte] Die mit dieser Technologie erreichbare Energiedichte, wenn der Energiespiecher vollständig entladen und aufgeladen wird. Bei teilweiser Entladung lässt sich die Energiedichte durch Multiplikation mit der Entladetiefe (englisch: \emph{Depth of Discharge – DoD}) abschätzen.\\
	Einheit: $\frac{Wh}{l}$
	\item[Spezifische Energie] Die mit dieser Technologie erreichbare spezifische Energie.\\
	Einheit: $\frac{Wh}{kg}$
	...
	Einheit: $\frac{W}{kg}$
	\item[Leistungsdichte, Anfahren] Für die Berechnung wird die maximale Leistung verwendet, die für 10s erreicht werden kann.\\
	Einheit: $\frac{W}{kg}$
	\item[Spezifische Leistung, Anfahren] Für die Berechnung wird die maximale Leistung verwendet, die bei für 10s erreicht werden kann.\\
	Einheit: $\frac{W}{l}$
	\item[Effizienz] Das Verhältnis zwischen verwendeter und abgerufener elektrischer Energie, abhängig vom Ladezustand.\\
	Einheit: 1
	\item[Ladezeit] Die maximale Leistung, mit der kontinuierlich geladen werden kann.\\
	Einheit: $\widetilde{C}\hat{=} \frac{W}{Wh}$
\end{description}
